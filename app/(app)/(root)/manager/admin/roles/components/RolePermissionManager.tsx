...
export default function RolePermissionManager({ 
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const permissionsResponse = await getPermissions({ size: 1000 });
      setPermissions(permissionsResponse.data);

      // Fetch assigned permissions for this role
      const assignedPermissionsResponse = await getPermissionsByRole(role.id);
      const assignedPermissionIds = assignedPermissionsResponse.map(p => p.id);
      setAssignedPermissions(assignedPermissionIds);

      // For now, we'll keep features empty since we're focusing on permissions
      setAssignedFeatures([]);
    } catch (error: any) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Assign permissions directly to role
      await assignRolePermissions(role.id, assignedPermissions);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            {t('roleCode')}: <Badge variant="light" color="info">{role.code}</Badge>
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">{t('assignedFeatures')}</p>
                <p className="text-2xl font-bold">{assignedFeatures.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
